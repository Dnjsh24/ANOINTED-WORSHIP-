...
import { Check, CheckCircle2, ChevronDown, Lock, MapPin, Music2, Users, Loader2 } from "lucide-react";
import Link from "next/link";
import { useState, useEffect } from "react";
import { joinTeamAction, getTeamPreviewAction } from "@/app/actions";
          ...
          <form action={joinTeamAction} className="mt-7 space-y-5">
            ...
            <div>
              ...
              {code && !codeValid && !searching && code.trim().length >= 5 && (
                <p className="mt-2 text-xs font-semibold text-red-400 text-left">
            ...
            {codeValid && teamPreview && (
              <div className="mx-auto flex items-center gap-4 rounded-xl border border-white/[0.08] bg-[#17161b] p-4 text-left max-w-md w-full animate-fade-in">
                <div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-violet-500/15 border border-violet-400/20">
                ...
                <div>
                  ...
                  <div className="mt-1.5 flex items-center gap-3 text-[10px] font-bold text-zinc-500">
                    <span className="flex items-center gap-1"><Users className="size-3" /> {teamPreview.members} Members</span>
                    <span className="flex items-center gap-1"><MapPin className="size-3" /> {teamPreview.serviceTime}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                <select
                  name="role"
                  required
                  className="w-full appearance-none rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-semibold text-white focus:border-violet-400/50 focus:outline-none focus:ring-1 focus:ring-violet-400/30 transition-all"
                >
                  {ROLES.map(r => <option key={r.label} value={r.value} className="bg-[#111014]">{r.label}</option>)}
                </select>
