...
export default function KineticCanvas({ blocks, settings, slide, onUpdateBlock, onUpdateBlocks, playKey = 0, selectedBlockIds = [], onSelectBlock, sceneLayers = [], selectedSceneLayerId, selectedSceneLayerIds = [], onSelectSceneLayer, onUpdateSceneLayer, onUpdateSceneLayers }: KineticCanvasProps) {
  ...
  return (
    ...
    >
      ...
      {sceneLayers.filter((layer) => !layer.hidden).map((layer) => {
        ...
        return <button key={layer.id} type="button" onPointerDown={(event) => { event.stopPropagation(); onSelectSceneLayer?.(layer.id, event.shiftKey || event.ctrlKey || event.metaKey); if (!layer.locked) { setDraggingSceneLayer(layer); setInitialSceneLayers(layer.groupId ? sceneLayers.filter((candidate) => candidate.groupId === layer.groupId && !candidate.locked) : [layer]); setStartPos({ x: event.clientX, y: event.clientY }); setStartBlockPos({ x: layer.x, y: layer.y }); } }} className={cn("absolute overflow-hidden text-left", selectedSceneLayerIds.includes(layer.id) && "ring-2 ring-violet-400", isCurrentlyPlaying && entrance && "fill-mode-both", isCurrentlyPlaying && entrance)} style={{ left: `${layer.x}%`, top: `${layer.y}%`, width: `${layer.width}%`, height: `${layer.height}%`, transform: `rotate(${layer.rotation}deg)`, opacity: layer.opacity ?? 1, backgroundColor: layer.kind === "shape" ? layer.backgroundColor : undefined, borderColor: layer.borderColor, borderStyle: layer.borderWidth ? "solid" : undefined, borderWidth: layer.borderWidth, borderRadius: layer.shapeType === "ellipse" ? "50%" : `${layer.borderRadius || 0}px`, clipPath: layer.shapeType === "triangle" ? "polygon(50% 0, 100% 100%, 0 100%)" : undefined, zIndex: 20 + (layer.zIndex ?? 0), animationDelay: isCurrentlyPlaying && entrance ? `${(layer.startTime || 0) + motion.entranceDelay}s` : undefined, animationDuration: isCurrentlyPlaying && entrance ? `${motion.entranceDuration}s` : undefined }}>
          <span className={cn("block h-full w-full", isCurrentlyPlaying && exit && "fill-mode-forwards", isCurrentlyPlaying && exit)} style={{ animationDelay: isCurrentlyPlaying && exit && layer.duration ? `${(layer.startTime || 0) + layer.duration + motion.exitDelay}s` : undefined, animationDuration: isCurrentlyPlaying && exit ? `${motion.exitDuration}s` : undefined }}>
            {layer.kind === "text" && <span className="block h-full w-full whitespace-pre-wrap" style={{ color: layer.color || "#fff", backgroundColor: layer.backgroundColor, fontFamily: layer.fontFamily, fontSize: `${Math.max(10, (layer.fontSize || 56) * .4)}pt`, fontWeight: layer.bold ? 700 : 400, fontStyle: layer.italic ? "italic" : "normal", textDecoration: layer.underline ? "underline" : "none", textAlign: layer.textAlign || "left" }}>{layer.text || "Text"}</span>}
            ...
            {layer.kind === "video" && layer.mediaUrl && <video src={layer.mediaUrl} className="h-full w-full" style={{ objectFit: layer.objectFit || "cover" }} muted />}
      ...
      {/* Blocks */}
      {(() => {
        // Safe max font size (pt) is constrained by horizontal (approx 2200/chars) and vertical (approx 810/lines)
        const maxBlockLength = Math.max(...blocks.map(b => b.text.length), 1);
        const hFit = 2200 / maxBlockLength;
        const vFit = 810 / (Math.max(1, blocks.length) * 1.3);
        ...
        return (
          <div
            ...
            className={cn(
              "absolute origin-center cursor-move select-none p-1 transition-colors z-10 w-max text-center whitespace-pre",
