...
export function isBibleTranslation(value: unknown): value is BibleTranslation {
  return BIBLE_TRANSLATIONS.some((translation) => translation.id === value);
}

export function bibleBookChapterCount(bookName: string) {
  return BIBLE_BOOKS.find((book) => book.name === bookName)?.chapters ?? 0;
}
