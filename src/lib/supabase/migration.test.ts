...
const remotePairingMigration = readFileSync(
  join(process.cwd(), "supabase", "migrations", "20260726134512_worship_remote_pairing_sessions.sql"),
  "utf8",
);
const oneTimeRemotePairingMigration = readFileSync(
  join(process.cwd(), "supabase", "migrations", "20260726200156_make_worship_remote_pairing_one_time.sql"),
  "utf8",
);
const privateRemoteHelperMigration = readFileSync(
  join(process.cwd(), "supabase", "migrations", "20260726201305_move_remote_policy_helper_to_private_schema.sql"),
  "utf8",
);
...
describe("Supabase migration", () => {
  ...
  it("keeps Remote pairing one-time and uses a non-API role helper", () => {
    expect(remotePairingMigration).toContain("public.is_worship_remote_operator");
    expect(oneTimeRemotePairingMigration).toContain("session_row.paired_by is not null");
