...
const MINOR_KEYS = ["Cm", "C#m", "Dbm", "Dm", "D#m", "Ebm", "Em", "Fm", "F#m", "Gbm", "Gm", "G#m", "Abm", "Am", "A#m", "Bbm", "Bm"];
...
export function ChangeKeyButton({
  ...
  const effectiveKey = localKey ?? currentKey ?? originalKey ?? "C";
  const isMinor = (originalKey || effectiveKey)?.endsWith("m");
  const baseKeys = isMinor ? MINOR_KEYS : MAJOR_KEYS;
  const activeKeys = baseKeys.includes(effectiveKey) ? baseKeys : [...baseKeys, effectiveKey];

  const handleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newKey = e.target.value;
    if (newKey !== effectiveKey) {
      setLocalKey(newKey);
      setIsUpdating(true);
      ...
      const res = await updateSetlistSongKeyAction(formData);
      if (res && !res.ok) {
