-- Let desktop setlists sync without manufacturing a Timeline event.
-- Forward-only function replacement. If rollback is required, deploy a new
-- migration restoring the prior apply_worship_mutation implementation.
...
begin
  ...
  if p_command in ('song.create', 'song.update') then
    ...
    v_song_id := (p_payload->>'id')::uuid;
    v_event_id := nullif(p_payload->>'eventId', '')::uuid;
    ...
    if v_event_id is not null then
      if not exists (select 1 from public.events where id = v_event_id and team_id = v_team_id) then
        raise exception 'Timeline event is unavailable' using errcode = '42501';
      end if;
    end if;
