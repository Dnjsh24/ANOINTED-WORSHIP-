...
create or replace function public.is_worship_remote_operator(p_team_id uuid)
...
with check (created_by = auth.uid() and public.is_worship_remote_operator(team_id));
...
begin
  ...
  if not found or session_row.revoked_at is not null or session_row.expires_at <= now()
  ...
  if not public.is_worship_remote_operator(session_row.team_id) then raise exception 'You do not have Remote permission for this team'; end if;
